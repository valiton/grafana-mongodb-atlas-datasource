...
export const HUMANITZED_DIMENSIONS = {
  'disk_measurements': DIMENSIONS['disk_measurements'].reduce((map, dim) => ({
    [humanize(dim)]: dim,
    ...map
  }), {}),
  'database_measurements': DIMENSIONS['database_measurements'].reduce((map, dim) => ({
    [humanize(dim)]: dim,
    ...map
  }), {}),
  'process_measurements': DIMENSIONS['process_measurements'].reduce((map, dim) => ({
    [humanize(dim)]: dim,
    ...map
  }), {})
}
...
export class GrafanaMongoDbQueryParameterCtrl {
  ...
  constructor($scope, templateSrv, uiSegmentSrv, datasourceSrv, $q) {
    ...
    $scope.init = () => {
      ...
      $scope.projectSegment = uiSegmentSrv.getSegmentForValue(target.projectName, 'select project');
      $scope.clusterSegment = uiSegmentSrv.getSegmentForValue(target.clusterName, 'select cluster');
      $scope.mongoSegment = uiSegmentSrv.getSegmentForValue(target.mongo, 'select mongo');
      $scope.databaseSegment = uiSegmentSrv.getSegmentForValue(target.database, 'select database');
      $scope.diskSegment = uiSegmentSrv.getSegmentForValue(target.disk, 'select disk');
      $scope.metricsSegment = uiSegmentSrv.getSegmentForValue(target.metricName, 'select metric');

      $scope.dimensionSegment = uiSegmentSrv.getSegmentForValue(target.dimensionName, 'select dimension');
    }
    
    $scope.getMetrics = () => Promise.resolve(Object.entries(this.metricMapping).map(([value, key]) => ({
      key,
      value
    })))

    $scope.getProjects = () => {
      return $scope.datasource.metricFindQuery('projects')
        .then(projects => {
          this.projectNameIdMapping = projects.reduce((map, project) => ({
            ...map,
    ...
    $scope.getClusters = () => {
      return $scope.datasource.metricFindQuery('clusters', {
        projectId: $scope.target.projectId
      })
        .then(clusters => {
          this.clusterNameIdMapping = clusters.reduce((map, cluster) => ({
            ...map,
    ...
    $scope.getMongos = () => {
      return $scope.datasource.metricFindQuery('mongos', {
        projectId: $scope.target.projectId,
    ...
    $scope.getDimensions = () => {
      console.log('HUPFI', $scope.target.metricId)
      return Promise.resolve(Object.entries(HUMANITZED_DIMENSIONS[$scope.target.metricId]).map(([value, key]) => ({
        key,
        value
      })))
    }
    ...
    $scope.projectChanged = () => {
      ...
      $scope.target.projectId = this.projectNameIdMapping[$scope.projectSegment.value];
      $scope.onChange();
    };
    $scope.clusterChanged = () => {
      $scope.target.clusterName = $scope.clusterSegment.value;
      $scope.target.clusterId = this.clusterNameIdMapping[$scope.clusterSegment.value];
      $scope.onChange();
    };
    $scope.metricChanged = () => {
    ...
    $scope.mongoChanged = () => {
      $scope.target.mongo = $scope.mongoSegment.value;
      $scope.onChange();
    };
    $scope.diskChanged = () => {
    ...
    $scope.dimensionChanged = () => {
      ...
      console.log(HUMANITZED_DIMENSIONS, HUMANITZED_DIMENSIONS[$scope.target.metricId], $scope.target.metricId)
      $scope.target.dimensionId = HUMANITZED_DIMENSIONS[$scope.target.metricId][$scope.target.dimensionName];
      $scope.onChange();
    };
...
export function grafanaMongodbAtlasQueryParameter() {
  return {
    templateUrl: 'public/plugins/grafana-mongodb-atlas-datasource/partials/query.parameter.html',
...
coreModule.directive('grafanaMongodbAtlasQueryParameter', grafanaMongodbAtlasQueryParameter);
